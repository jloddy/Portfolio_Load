/*---------------------------------------------------------------------
	...
	PURPOSE: IMPORT CLIENT DEBT FILES (PIPE DELIMITED)
	...
	STEPS:
		...
		4) COMBINES CLIENT FILES AND OUTPUTS TO OUTPUT LIBRARY

-----------------------------------------------------------------------*/
OPTIONS MPRINT MLOGIC SYMBOLGEN VALIDVARNAME=ANY;

FILENAME GEO_IN PIPE 'DIR "E:\SHARED\CADA\SAS SOURCE CODE\DEVELOPMENT\JHUBBARD\EBG\1_PORTFOLIO_FILE_STANDARDIZATION\PRODUCTION ENVIRONMENT TEST\STANDARDIZED CLIENT FILES\*.*" /B ';

%LET FILE_LOC_IN = "E:\SHARED\CADA\SAS SOURCE CODE\DEVELOPMENT\JHUBBARD\EBG\1_PORTFOLIO_FILE_STANDARDIZATION\PRODUCTION ENVIRONMENT TEST\STANDARDIZED CLIENT FILES";
...
%MACRO CHECK_FIRST_FILE_IN(FILE=,TOTAL_COUNTER=,FILE_NAME=,EXTENSION=,LAST_CLIENT,CLIENT_NAME);
...
DATA _NULL_;
	SET COUNT_FILES_IN;
	CALL EXECUTE('%NRSTR(%CHECK_FIRST_FILE_IN(FILE='||FILE_NAME_FULL||',TOTAL_COUNTER='||PUT(COUNT,8.)||',FILE_NAME='||FILE_NAME||',EXTENSION='||EXTENSION||',LAST_CLIENT='||LAST_CLIENT_DATA||',CLIENT='||CLIENT_NAME||'))');
RUN;
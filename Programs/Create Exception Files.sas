...
DATA MILITARY_PRE(KEEP=SSN DOB LASTNAME FIRSTNAME ISSUER_ACCOUNT_NUMBER INDICATOR);
	SET MILITARY_INPUT;
	SSN = COMPRESS(COMPRESS(F1,"1234567890","kis"));
	DOB = F2;
	LASTNAME = UPCASE(F3);
	FIRSTNAME = UPCASE(F4);
	ISSUER_ACCOUNT_NUMBER = COMPRESS(COMPRESS(F5,"1234567890","kis"));
	INDICATOR = COMPRESS(F7);
	IF INDICATOR ^= 'NNN' THEN OUTPUT MILITARY_PRE;
RUN;
...
PROC SQL;
	CREATE TABLE MILITARY_FINAL_EXCEPTIONS AS
	SELECT B.ACCOUNTID			AS ACCOUNT_ID
		  ,A.SSN
		  ,A.DOB
	FROM MILITARY_PRE A
